...
// 👇 Prop 'contractToEdit' add kiya
const UploadContractForm = ({ onClose, onContractAdded, contractToEdit = null }) => {
    ...
    // 👇 Effect: Agar Edit mode hai, to purana data fill karo
    useEffect(() => {
        if (contractToEdit) {
            setFormData({
                vendorId: contractToEdit.vendor?._id || contractToEdit.vendor || '',
                contractTitle: contractToEdit.contractTitle,
                // Date format fix for input type="date" (YYYY-MM-DD)
                start_date: new Date(contractToEdit.start_date).toISOString().split('T')[0],
                end_date: new Date(contractToEdit.end_date).toISOString().split('T')[0],
                contractValue: contractToEdit.contractValue,
                renewalStatus: contractToEdit.renewalStatus,
                paymentFrequency: contractToEdit.paymentFrequency,
                contactPerson: contractToEdit.contactPerson || '',
                uploadedFileName: contractToEdit.documentPath ? 'Existing Document (Upload new to replace)' : ''
            });
        }
    }, [contractToEdit]);
    ...
    const handleChange = (e) => {
        ...
        // Only auto-fill title if NOT editing (creating new)
        if (name === 'vendorId' && !contractToEdit) {
            const vendor = vendors.find(v => v._id === value);
            if (vendor) {
                setFormData(prev => ({ 
                    ...prev, 
                    contractTitle: vendor.productTool || '',
                    contactPerson: prev.contactPerson || vendor.contactPerson?.name || ''
                }));
            }
        }
    };
    ...
    const handleFileChange = (e) => {
        ...
        setFormData(prev => ({ ...prev, uploadedFileName: file ? file.name : '' }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // 👇 Logic: POST (Create) vs PUT (Update)
            let res;
            if (contractToEdit) {
                // Update Mode
                res = await axios.put(`${API_BASE_URL}/api/contracts/${contractToEdit._id}`, data, {
                    headers: { 'x-auth-token': token }
                });
            } else {
                // Create Mode
                res = await axios.post(`${API_BASE_URL}/api/contracts`, data, {
                    headers: { 'x-auth-token': token }
                });
            }

            setSuccess(true);
            onContractAdded(res.data); // Notify parent
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl transform transition-all duration-300 scale-100 opacity-100 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center p-5 border-b border-gray-200">
                    <div>
                        ...
                        <h3 className="text-xl font-bold text-gray-900">
                            {contractToEdit ? 'Edit Contract Details' : 'Upload New Contract'}
                        </h3>
                        <p className="text-sm text-gray-500">
                            {contractToEdit ? 'Update existing contract information.' : 'Add a new vendor contract to the system.'}
                        </p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    {success && <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative">
                        {contractToEdit ? 'Contract updated successfully!' : 'Contract uploaded successfully!'}
                    </div>}
                    ...
                    <div className="grid grid-cols-2 gap-x-5 gap-y-4">
                         ...
                         <div>
                            ...
                            <select name="vendorId" value={formData.vendorId} onChange={handleChange} required className="w-full border border-gray-300 rounded-lg p-2.5 text-sm">
                        ...
                        <div>
                             ...
                             <input type="text" name="contractTitle" value={formData.contractTitle} onChange={handleChange} required className="w-full border border-gray-300 rounded-lg p-2.5 text-sm" />
                        </div>

                        {/* Dates */}
                        <div><label className="block text-sm font-medium text-gray-700 mb-1">Start Date *</label><input type="date" name="start_date" value={formData.start_date} onChange={handleChange} required className="w-full border border-gray-300 rounded-lg p-2.5 text-sm" /></div>
                        <div><label className="block text-sm font-medium text-gray-700 mb-1">End Date *</label><input type="date" name="end_date" value={formData.end_date} onChange={handleChange} required className="w-full border border-gray-300 rounded-lg p-2.5 text-sm" /></div>
                        
                        {/* Value */}
                        <div><label className="block text-sm font-medium text-gray-700 mb-1">Value *</label><input type="number" name="contractValue" value={formData.contractValue} onChange={handleChange} required className="w-full border border-gray-300 rounded-lg p-2.5 text-sm" /></div>

                        {/* Payment Frequency */}
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Payment Frequency</label>
                            <select name="paymentFrequency" value={formData.paymentFrequency} onChange={handleChange} className="w-full border border-gray-300 rounded-lg p-2.5 text-sm">
                                <option>Monthly</option><option>Quarterly</option><option>Annually</option><option>One-Time</option>
                            </select>
                        </div>
                        ...
                        <div className="col-span-2">
                            ...
                            <input type="text" name="contactPerson" value={formData.contactPerson} onChange={handleChange} className="w-full border border-gray-300 rounded-lg p-2.5 text-sm" />
                        </div>
                        ...
                        <div className="col-span-2">
                            ...
                            <div className="flex items-center space-x-3">
                                <label className="cursor-pointer bg-white py-2 px-4 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center">
                                    <Upload className="w-4 h-4 mr-2" /> {contractToEdit ? 'Replace File' : 'Choose File'}
                                    <input type="file" className="sr-only" onChange={handleFileChange} accept=".pdf,.doc,.docx,.png,.jpg,.jpeg,.xls,.xlsx,.csv,.txt" />
                                </label>
                                <span className="text-sm text-gray-500">{formData.uploadedFileName || 'No file chosen'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                        ...
                        <button type="submit" disabled={loading} className="px-6 py-2 bg-sky-600 text-white rounded-lg">
                            {loading ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : (contractToEdit ? 'Update Contract' : 'Upload Contract')}
                        </button>
                    </div>
                </form>
            </div>
